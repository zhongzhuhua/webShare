...
exports.copy = function(root, dir, callback) {
  var files = getAllFilesDir(root);
  files.forEach(function(file) {
    var stat = fs.lstatSync(file);
    if (stat.isDirectory()) {
      try {
        console.log(file.replace(root, dir));
        fs.mkdirSync(file.replace(root, dir));
      } catch (e) {}
    }
  });
  copyDir(root, dir, callback);
};

function getAllFilesDir(root, dir) {
  var res = [];
  ...
  files.forEach(function(file) {
    ...
    if (!stat.isDirectory()) {
      ...
      res = res.concat(getAllFilesDir(pathname, dir));
    }
  });
