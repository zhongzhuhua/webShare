...
(function(ice) {
  ice.simpledate = function(options) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      var datas = {
        year: _dyear($dom.getAttribute('data-year')),
        month: _dmonth($dom.getAttribute('data-month')),
        date: _ddate($dom.getAttribute('data-date'))
      };
      ...
      $date.addEventListener(ice.tapClick, function(e) {
        ...
        if (clazz.indexOf('ic-date') > -1) {
          ...
          if (ice.isFunction(g.success)) {
            g.success({
              getValue: function() {
                var val = ele ? ele.getAttribute('data-value') : g.defValue;
    ...
    // 默认年
    function _dyear(v) {
      return ice.parseInt(v == null || v == '' ? g.defYear : v);
    };

    // 默认月
    function _dmonth(v) {
      return ice.parseInt(v == null || v == '' ? g.defMonth : v);
    };

    // 默认日
    function _ddate(v) {
      return ice.parseInt(v == null || v == '' ? g.defDate : v);
    };
  };
})(ice);
